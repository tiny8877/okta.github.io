...
`request`:
  ...
  * You must sign the JWT using either the app's client secret, or a private key whos public key is registered on the app's JWKSet.
  * The JWT can't be encrypted.
  * Okta supports the [HMAC](https://tools.ietf.org/html/rfc7518#section-3.2), [RSA](https://tools.ietf.org/html/rfc7518#section-3.3) and [ECDSA](https://tools.ietf.org/html/rfc7518#section-3.4) signature algorithms. HMAC signatures require that the client be using a `token_endpoint_auth_method` which uses a `client_secret`, RSA and ECDSA signatures requires that the client have registered a public key.
...
* The private key that used to sign with must have the corresponding public key registered in the clients [JWKSet](oauth-clients#json-web-key-set).
* Sign the JWT with one of the following signature algorithms: RS256, RS384, RS512, ES256, ES384 and ES512.
