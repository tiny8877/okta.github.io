...
| value      | String   | The actual base64-encoded hashed password (for `SHA-256`). The actual radix64-encoded hashed password (for `BCRYPT`)                                                                   | TRUE                               | N/A                | N/A                |
| salt       | String   | Specifies the base64-encoded password salt used to generate the hash (for `SHA-256`). Specifies the radix64-encoded password salt used to generate the hash (for `BCRYPT`)                                                       | TRUE                               | 22 (only for `BCRYPT` algorithm) | 22 (only for `BCRYPT` algorithm) |
