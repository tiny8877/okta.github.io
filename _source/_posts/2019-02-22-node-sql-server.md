...
The `inert` plugin is used to serve static files and `vision` adds support for rending server-side templates. Here `ejs` is configured as the template engine.
