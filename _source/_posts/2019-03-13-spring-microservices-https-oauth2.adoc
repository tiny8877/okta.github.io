...
Shut down all your Docker containers before proceeding to the next section.
...
At this point, you should be able to start all your apps by running `./mvnw spring-boot:start` in each project (in separate terminal windows).
...
Run `mvn clean install` to rebuild all your Docker images. Then stop and restart all containers.
...
Now all your apps are running in Docker with HTTPS!
...
Add a `src/main/resources/bootstrap.yml` file that allows this instance to read its configuration from Spring Cloud Config.
...
public class SchoolController {
    ...
    public ResponseEntity<List<TeachingClassDto>> listClasses(
        ...
        restTemplate.getInterceptors().add(getBearerTokenInterceptor(accessToken.getTokenValue())); // <4>
