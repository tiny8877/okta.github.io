...
You can register for a https://developer.okta.com/signup/[free developer account] that will allow you to have up to 1000 monthly active users for $0. That should be plenty for this example. After creating your account, create a new Web Application in Okta's dashboard (**Applications** > **Add Application**). Give the app a name you'll remember, duplicate the existing Login redirect URI and make it use HTTPS. Click **Done**.
...
HTTPS stands for "Secure" HTTP. HTTPS connections are encrypted and its contents are vastly more difficult to read than HTTP connections. There's been a big movement in recent years to use HTTPS everywhere, even when developing. There's issues you might run into when running with HTTPS, and it's good to catch them early.
...
Also, make sure to change the Eureka address to be `https://localhost:8761/eureka`.
...
Docker doesn't read from environment variables, nor does it know about your local CA (Certificate Authority) that `mkcert` modifies.
...
Then create an `.env` file with environment variables for Spring Boot and HTTPS.
...
Run `mvn clean install` to rebuild all your Docker images with HTTPS enabled for Eureka registration. Then start all your service containers.
...
Let's make one more security improvement and add OAuth 2.0 security to your school-service API.

== API Security with OAuth 2.0

Add the Okta Spring Boot Starter to `school-service/pom.xml`:
...
Then copy `config/school-ui.properties` and `school-ui-production.properties` to have a `school-service` equivalent.

[source,shell]
----
cp config/school-ui.properties config/school-service.properties
cp config/school-ui-production.properties config-data/school-service.properties
----

TIP: Yes, you could create a Service app on Okta that uses client credentials, but this post is already complicated enough. See link:/blog/2018/04/02/client-creds-with-spring-boot[Secure Server-to-Server Communication with Spring Boot and OAuth 2.0] for more information on that approach.
...
public class SchoolController {
    ...
    public SchoolController(OAuth2AuthorizedClientService clientService, RestTemplate restTemplate) { // <1>
...
At this point, you can choose to run all your apps individually with `./mvnw spring-boot:run` or with Docker Compose. The latter method requires only two commands, if you'd like to try them.
...
== Use HTTP Basic Auth for Secure Communication with Eureka and Spring Cloud Config

To improve security between your microservices, Eureka Server, and Spring Cloud Config even more, you can add HTTP Basic Authentication. To do this, you'll want to add `spring-boot-starter-security` as a dependency to both the `config` project and `discovery`. Then you'll want to specify a `spring.security.user.password` for each and encrypt it. You can learn more about how to do this in https://cloud.spring.io/spring-cloud-config/single/spring-cloud-config.html#_security[Spring Cloud Config's security docs].
...
* link:/blog/2018/05/17/microservices-spring-boot-2-oauth[Develop a Microservices Architecture with OAuth 2.0 and JHipster]
...
_Got questions, ask them in the comments below! If your question doesn't relate to this post, please post them to our https://devforum.okta.com/[Developer Forums]._
