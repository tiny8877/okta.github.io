...
This functionality can be used to add data that is sensitive, calculated, or only available in real time, or to add complex data structures not appropriate for Okta user profiles. Data added this way is never logged stored by Okta. As an example, tokens minted for a medical app can be augmented with confidential patient data provided by your external service and not stored in Okta.
